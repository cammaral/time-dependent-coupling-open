...
def selected_ep_points(ep_list):
    """
    Usa apenas 3 pontos do sweep em epsilon:
    - inicial
    - meio
    - máximo/final
    """
    idxs = [0, len(ep_list) // 2, len(ep_list) - 1]
    rows = []
    for name, idx in zip(["inicial", "meio", "maximo"], idxs):
        rows.append({
            "ep_label": name,
            "ep_index": int(idx),
            "epsilon_value": float(ep_list[idx]),
        })
    return rows
...
args = {
    ...
    "gamma_phi": 1e-2,
    "coupling": "gauss",
    # gauss4: agora T fica fixo e quem varia é epsilon
    "epsilon": None,
    "T": 7.5,
}


extra = "gauss4_epmax_T75_wigner_all_times_open_only_obs_video"
...
#======= GAUSS4 ========
# Agora o sweep é em epsilon, com T fixo em 7.5.
epmax = 10 / (50 / 15)
ep_list = np.linspace(6 / (50 / 15), epmax, 250, endpoint=True)
ep_points = selected_ep_points(ep_list)
...
# ==========================
save_dir = make_exp_folder(base_name=f"wigner_{args['coupling']}_gauss4_ep_all_times", root="results")
...
with open(os.path.join(save_dir, "run_info.txt"), "w", encoding="utf-8") as f:
    ...
    f.write(f"T fixo: {float(args_init['T'])}\n")
    f.write(f"epmax: {float(epmax)}\n")
    f.write(f"len(ep_list original): {len(ep_list)}\n")
    f.write("ep_points usados:\n")
    for row in ep_points:
        f.write(f"  {row['ep_label']}: index={row['ep_index']}, epsilon={row['epsilon_value']}\n")
...
np.save(os.path.join(save_dir, "ep_list_original.npy"), ep_list)
pd.DataFrame(ep_points).to_csv(os.path.join(save_dir, "ep_points_used.csv"), index=False)
...
# ==========================
# OPEN HAMILTONIAN - ONLY 3 EPSILON POINTS, T FIXED
# ==========================
args_per_ep = []

for row in tqdm(ep_points, desc="epsilon points"):
    ep_label = row["ep_label"]
    ep_index = row["ep_index"]
    epsilon_value = row["epsilon_value"]

    args["epsilon"] = float(epsilon_value)
    args["T"] = 7.5

    print(
        f"Rodando variável aberto | {ep_label} | "
        f"ep_index={ep_index} | epsilon={epsilon_value} | T={args['T']}"
    )
    ...
    label = f"var_aberto_ep_{ep_label}_idx{ep_index:03d}_ep{epsilon_value:.4f}_T{args['T']:.2f}"
...
pd.DataFrame(args_per_ep).to_csv(os.path.join(save_dir, "args_per_ep_used.csv"), index=False)
