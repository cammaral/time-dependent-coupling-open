...
from quantum.non_classicality import entanglement
...
def make_exp_folder(base_name="entanglement_exp", root="results"):
...
#======= LINEAR/ EXP / TRIG =======
#wmax = 1/100
#w_list = np.linspace(-wmax, wmax, 100, endpoint=True)
#phi_max = np.pi/2
...
# ==========================
save_dir = make_exp_folder(base_name=f"entanglement_{args['coupling']}", root="results")
...
C_const_aberto = entanglement(sol_const_aberto.states)
C_const = entanglement(sol_const.states)
...
for ep in tqdm(T_list): #gauss
    ...
    C_var_aberto = entanglement(sol_var_aberto.states)
    C_var = entanglement(sol_var.states)
